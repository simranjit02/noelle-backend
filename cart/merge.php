...
if (empty($data['user_id']) || empty($data['items']) || !is_array($data['items'])) {
    ...
    echo json_encode(['status' => 'error', 'message' => 'User ID and items array required']);
    exit();
}
...
foreach ($items as $item) {
    ...
    $product_id = strval($item['product_id']); // Keep as string - product_id is VARCHAR
...
echo json_encode([
    'status' => 'success',
    'message' => "Merged $merged_count items into user cart",
